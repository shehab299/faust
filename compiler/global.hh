...
struct global {
    ...
    std::vector<std::string>
        gImportDirList;  // dir list enrobage.cpp/fopensearch() searches for imports, etc.
    std::vector<std::string> gArchitectureDirList;  // dir list enrobage.cpp/fopensearch() searches
    ...
    std::string gProcessName;     // -pn option, name of the entry point of the Faust program, by
                                  // default 'process'
    std::string gSuperClassName;  // -scn option, name of the root class the generated dsp class
    ...
    bool
        gUseDefaultSound;  // If default global variable is used in 'soundfile' primitive generation
    bool        gHasTeeLocal;  // For wast/wasm backends
    std::string gFastMathLib;  // -fm faster version of some mathematical functions (pow/exp/log),
                               // the fastmath code mapping file
...
#ifdef WASM_BUILD
    // One single global visitor for WebAssembly, so that sub-containers and the global container
    // use the same heap
    ...
    // One single global visitor Interpreter backend, so that sub-containers and the global
    // container use the same heap
    DispatchVisitor* gInterpreterVisitor;
#endif

#ifdef JULIA_BUILD
    // One single global visitor Julia backend, so that sub-containers and the global container use
    // the same heap
    JuliaInstVisitor* gJuliaVisitor;
#endif

#ifdef JSFX_BUILD
    // One single global visitor JSFX backend, so that sub-containers and the global container use
    // the same heap
