...
LIBFAUST_API int http_fetch(const char* url_tmp, char** fileBuf)
{
    ...
    do {
        ...
        if (charIndex == NULL) {
            ...
            tempSize = (int)strlen("GET ") + (int)strlen(charIndex) + (int)strlen(HTTP_VERSION) + 4;
            /* + 4 is for ' ', '\r', '\n', and NULL */
            if (_checkBufSize(&requestBuf, &bufsize, tempSize) ||
        ...
        if (!hideUserAgent && userAgent == NULL) {
            tempSize = (int)strlen("User-Agent: ") + (int)strlen(DEFAULT_USER_AGENT) +
    ...
    if (charIndex != NULL) {
        ret = sscanf(charIndex + strlen("content-length: "), "%d", &contentLength);
        if (ret < 1) {
            close(sock);
            errorSource = FETCHER_ERROR;
...
const char* http_strerror()
{
    if (errorSource == ERRNO) {
        ...
        if (strstr(http_errlist[http_errno], "%d") == NULL) {
            ...
            snprintf(&convertedError[strlen(convertedError)], 8, "%d", errorInt);
            stringIndex += 2; /* Skip past the %d */
